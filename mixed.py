...
def colleastsq(VBhat, VEhat, p, colCounts, XTX, XTY, XTE, ETY):
    ...
    for colid in colCounts.keys():
    	XTEWETX = np.add(XTEWETX,np.outer(XTE[colid],XTE[colid]/float(VEhat+VBhat*colCounts[colid])))
    	XTEWETY = np.add(XTEWETY,XTE[colid]*ETY[colid]/float(VEhat+VBhat*colCounts[colid]))
...
def rowleastsq(VAhat, VEhat, p, rowCounts, XTX, XTY, XTD, DTY):
    ...
    for rowid in rowCounts.keys():
    	XTDWDTX = np.add(XTDWDTX,np.outer(XTD[rowid],XTD[rowid]/float(VEhat+VAhat*rowCounts[rowid])))
    	XTDWDTY = np.add(XTDWDTY,XTD[rowid]*DTY[rowid]/float(VEhat+VAhat*rowCounts[rowid]))
...
def estimate(fileName):
    ...
    varU[2,2] = (muAhat-VAhat**2)*sumNid2*N**2+(muBhat-VBhat**2)*sumNdj2*N**2
    ...
    if rowcol==1:
        ...
        for line in fileObj:
            ...
            rowid = s[0]
            colid = s[1]
            varXTVAinvB -= 2*VAhat*VBhat*np.outer(XTD[rowid],XTE[colid])/(VEhat**2*(VEhat+VAhat*rowCounts[rowid]))
        ...
        for colid in colVals.keys(): 
            ...
            varXTVAinvB += VBhat*VAhat**2*np.outer(colVals[colid],colVals[colid])/(VEhat**2)                                      
        ...
        for line in fileObj:
            ...
            rowid = s[0]
            colid = s[1]
            varXTVBinvA -= 2*VBhat*VAhat*np.outer(XTE[colid],XTD[rowid])/(VEhat**2*(VEhat+VBhat*colCounts[colid]))
        ...
        for rowid in rowVals.keys():
            ...
            varXTVBinvA += VAhat*VBhat**2*np.outer(rowVals[rowid],rowVals[rowid])/(VEhat**2)   
    ...
    print("The variances of those estimates are %f, %f, and %f." %(varTheta[0,0],varTheta[1,1],varTheta[2,2]))
