...
def moments(fileName):
    ...
    VarUe += (muAhat - VAhat ** 2) * (
        N ** 2 * sumNid2
        - 2 * N * sum([i ** 3 for i in rowCounts.values()])
        + sum([i ** 4 for i in rowCounts.values()])
    )
    VarUe += 2 * VBhat ** 2 * (sumNdj2 ** 2 - sum([j ** 4 for j in colCounts.values()]))
    VarUe += (muBhat - VBhat ** 2) * (
        N ** 2 * sumNdj2
        - 2 * N * sum([j ** 3 for j in colCounts.values()])
        + sum([j ** 4 for j in colCounts.values()])
    )
    ...
    CovUaUe = (
        ...
        * VBhat ** 2
        * (
            sum([rowTs[i] ** 2 / float(rowCounts[i]) for i in rowCounts.keys()])
            - ZNm1p2
        )
    )
    CovUaUe += (muBhat - VBhat ** 2) * (
        N * sumNdj2 - N * rowCross - sum([j ** 3 for j in colCounts.values()]) + ZNm1p2
    )
    ...
    CovUbUe = (
        ...
        * VAhat ** 2
        * (
            sum([colTs[j] ** 2 / float(colCounts[j]) for j in colCounts.keys()])
            - ZNp2m1
        )
    )
    CovUbUe += (muAhat - VAhat ** 2) * (
        N * sumNid2 - N * colCross - sum([i ** 3 for i in rowCounts.values()]) + ZNp2m1
    )
    ...
    print(
        "Conservative variances of the estimates are %f, %f, and %f."
        % (VarVAhat, VarVBhat, VarVEhat)
    )
